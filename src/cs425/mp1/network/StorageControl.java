...
public class StorageControl {
    ...
    public static RPC getRPC(String key, ArrayList<String> params) {
        ...
        rawRPC.setMsg(String.format(rawRPC.getMsg(), params.toArray(new String[params.size()])));

        return rawRPC;
    }

    public static void sendRPC(String host, String key, ArrayList<String> params) {
        RPC rawRPC = availableRPCs.get(key);
        if (params.size() > 0)
            rawRPC.setMsg(String.format(rawRPC.getMsg(), params.toArray(new String[params.size()])));
